...
ITAD_KEY = server_env("game-encyclopedia")["ITAD_API_KEY"]
...
def itad_get(path):
    """Fetch an IsThereAnyDeal API resource with retries."""
    url = f"https://api.isthereanydeal.com{path}"
    delay = BACKOFF_BASE
    for _attempt in range(MAX_RETRIES):
        try:
            req = urllib.request.Request(
                url, headers={"User-Agent": "jasmeralia-backfill/1.0"}
            )
            ...
            if e.code in (429, 503):
                print(
                    f"  ITAD rate limit ({e.code}), backing off {delay:.0f}s...",
                    file=sys.stderr,
                )
                time.sleep(delay)
                delay *= 2
...
def strip_tracking(url: str) -> str:
    """Remove known affiliate and tracking query parameters."""
    ...
    return urllib.parse.urlunparse(
        parsed._replace(query=urllib.parse.urlencode(clean, doseq=True))
    )
    ...
    for _attempt in range(MAX_RETRIES):
        try:
            req = urllib.request.Request(
                ...
                data=data,
                headers={
                    "User-Agent": "jasmeralia-backfill/1.0",
            ...
            if e.code in (429, 503):
                print(
                    f"  ITAD prices rate limit ({e.code}), backing off {delay:.0f}s...",
                    file=sys.stderr,
                )
                time.sleep(delay)
                delay *= 2
...
def resolve_store_url(itad_redirect: str) -> str | None:
    ...
    try:
        req = urllib.request.Request(
            itad_redirect, headers={"User-Agent": "Mozilla/5.0"}
        )
...
def classify(game):
    ...
    plats = set()
    for link in links:
        if link.get("kind") != "download" or not link.get("url"):
            continue
        u = link["url"]
...
def main():
    """Find and optionally create missing PC store links."""
    ...
    games = DIRECTUS.fetch_all(
        "/items/games?fields=id,title,links.url,links.kind&sort=title"
    )
    ...
    for g in games:
        ...
        if "steam" in plats:
            if "gog" not in plats:
                steam_no_gog.append(g)
            if "epic" not in plats:
                steam_no_epic.append(g)
    ...
    def process_game(game, want_shop_ids: list[int]):
        ...
        for sid in want_shop_ids:
            ...
            # Sanity-check domain
            if sid == SHOP_GOG and "gog.com" not in real_url:
                continue
            if sid == SHOP_EPIC and "epicgames.com" not in real_url:
                continue
            if sid == SHOP_STEAM and "steampowered" not in real_url:
                continue

            shop_name = {SHOP_GOG: "gog", SHOP_EPIC: "epic", SHOP_STEAM: "steam"}[sid]
            print(
                f"  [{game['id']}] {title}  +{shop_name}: {real_url}", file=sys.stderr
            )
    ...
    for i, g in enumerate(steam_no_gog, 1):
        if i % 50 == 0:
            print(
                f"  [{i}/{len(steam_no_gog)}] checkpoint: gog_added={added_gog}",
                file=sys.stderr,
            )
    ...
    for i, g in enumerate(steam_no_epic, 1):
        if i % 50 == 0:
            print(
                f"  [{i}/{len(steam_no_epic)}] checkpoint: epic_added={added_epic}",
                file=sys.stderr,
            )
    ...
    for i, g in enumerate(epic_no_steam, 1):
        if i % 25 == 0:
            print(
                f"  [{i}/{len(epic_no_steam)}] checkpoint: steam_added={added_steam}",
                file=sys.stderr,
            )
    ...
    print(f"\n{'=' * 60}", file=sys.stderr)
    print(
        f"GOG links {'added' if args.apply else 'found'}:   {added_gog}",
        file=sys.stderr,
    )
    print(
        f"Epic links {'added' if args.apply else 'found'}:  {added_epic}",
        file=sys.stderr,
    )
    print(
        f"Steam links {'added' if args.apply else 'found'}: {added_steam}",
        file=sys.stderr,
    )
    if not args.apply:
