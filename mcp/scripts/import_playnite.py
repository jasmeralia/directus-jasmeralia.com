...
MANUAL_URLS = {
    ...
    (
        ...
        "GOG",
    ): "https://www.gog.com/en/game/broken_sword_shadow_of_the_templars_the_directors_cut",
    (
        "Consortium 2019 REBALANCE",
        "GOG",
    ): "https://www.gog.com/en/game/consortium_remastered",
    (
        "Daggerfall Unity - GOG Cut",
        "GOG",
    ): "https://www.gog.com/en/game/the_elder_scrolls_ii_daggerfall",
    ("Demon Stone", "GOG"): "https://www.gog.com/en/game/forgotten_realms_demon_stone",
    (
        "Fallout 2 Classic",
        "GOG",
    ): "https://www.gog.com/en/game/fallout_2_a_post_nuclear_role_playing_game",
    (
        "Fallout Tactics Classic",
        "GOG",
    ): "https://www.gog.com/en/game/fallout_tactics_brotherhood_of_steel",
    ...
    (
        ...
        "GOG",
    ): "https://www.gog.com/en/game/shadow_warrior_classic_redux",
    (
    ...
    (
        ...
        "GOG",
    ): "https://www.gog.com/en/game/the_whispering_valley_la_vallee_qui_murmure",
    (
        "The Witcher: Enhanced Edition",
        "GOG",
    ): "https://www.gog.com/en/game/the_witcher_enhanced_edition_directors_cut",
    # Epic
    ("DARQ", "Epic"): "https://store.epicgames.com/en-US/p/darq-complete-edition",
    (
        "Dying Light",
        "Epic",
    ): "https://store.epicgames.com/en-US/p/dying-light-enhanced-edition",
    (
        "Guild of Dungeoneering",
        "Epic",
    ): "https://store.epicgames.com/en-US/p/guild-of-dungeoneering-ultimate-edition",
    (
        "Halcyon 6",
        "Epic",
    ): "https://store.epicgames.com/en-US/p/halcyon-6-lightspeed-edition",
    ("Iratus", "Epic"): "https://store.epicgames.com/en-US/p/iratus-lord-of-the-dead",
    (
        "Monument Valley 2",
        "Epic",
    ): "https://store.epicgames.com/en-US/p/monument-valley-2-panoramic-edition",
    ("Q.U.B.E. 2", "Epic"): "https://store.epicgames.com/en-US/p/qube-2",
    (
        "Rustler - Grand Theft Horse",
        "Epic",
    ): "https://store.epicgames.com/en-US/p/rustler",
    (
        "Telltale Batman Season 1",
        "Epic",
    ): "https://store.epicgames.com/en-US/p/batman-the-telltale-series",
    (
        "The Textorcist",
        "Epic",
    ): "https://store.epicgames.com/en-US/p/the-textorcist-the-story-of-ray-bibbia",
    (
        "Tomb Raider I-III Remastered Starring Lara Croft",
        "Epic",
    ): "https://store.epicgames.com/en-US/p/tomb-raider-i-iii-remastered",
}
...
def _find_existing(
    title: str, key: str, db: dict, url_index: dict, store_url: str | None
) -> tuple[dict | None, str]:
...
def http_get(url, headers=None, timeout=10):
    """Fetch and decode JSON over HTTP."""
    h = {"User-Agent": UA}
    if headers:
        h.update(headers)
...
def http_post(url, data, headers=None, timeout=10):
    """Post JSON data and decode the HTTP response."""
...
def d_get(path, params=None):
    """Fetch a Directus resource with optional parameters."""
    url = f"{BASE}{path}"
    if params:
...
def normalize(s: str) -> list[str]:
    """Normalize a title into comparison tokens."""
    ...
    for _attempt in range(MAX_RETRIES):
        try:
            ...
            if e.code in (429, 403):
                print(
                    f"  [ITAD] rate limited (HTTP {e.code}), backing off {delay:.0f}s...",
                    flush=True,
                )
                time.sleep(delay)
                delay *= 2
            ...
            return None, 0.0
    print(
        f'  [ITAD] rate limit exceeded after {MAX_RETRIES} retries for "{title}"',
        flush=True,
    )
    return None, 0.0
...
def load_directus_games():
    """Load Directus games into title and URL indexes.
    ...
    for g in games:
        ...
        entry = {
            ...
            "link_kinds": {link["kind"] for link in links if link.get("kind")},
        }
...
def slug_from_title(title: str) -> str:
    """Convert a title into an import slug."""
    s = title.lower()
...
def main():
    """Import a Playnite export into Directus."""
    # Read CSV
    entries = []
    ...
    seen_keys: dict[str, str] = {}  # normalized_key → first title seen
    batch_anom: list[str] = []  # anomalies collected since last checkpoint
    total = len(entries)
    ...
    for i, entry in enumerate(entries, 1):
        ...
        if (title, source) in BLOCKED_TITLES:
            print("  → blocked (confirmed wrong match), skipped", flush=True)
            continue
        ...
        if store_url and url_conf < HIGH_CONF:
            stats["low_conf"] += 1
            review.append(
                {"title": title, "source": source, "url": store_url, "conf": url_conf}
            )
            print(
                f"  → low confidence ({url_conf:.2f}), queued for review: {store_url}"
            )
            if url_conf < 0.5:
                batch_anom.append(
                    f'VERY-LOW-CONF({url_conf:.2f}) [{source}] "{title}" → {store_url}'
                )

        if not store_url:
            stats["no_url"] += 1
            review.append({"title": title, "source": source, "url": None, "conf": 0.0})
            print("  → no URL found, queued for review")
        ...
        if existing:
            ...
            if already_has:
                ...
                stats["link_added"] += 1
                print(
                    f"  → {'ADDED' if not DRY_RUN else 'DRY'} link to game {existing['id']}: {store_url}"
                )
            ...
            if store_url and url_conf >= HIGH_CONF:
                ...
                print(
                    f"  → {'CREATED' if not DRY_RUN else 'DRY'} new game + link: {store_url}"
                )
            else:
                stats["no_url"] += 1
                print("  → skipped (no high-confidence URL)")
    ...
    # Write review file
    with open(REVIEW_FILE, "w", encoding="utf-8") as review_file:
        review_file.write("# Playnite Import Review\n\n")
        review_file.write(
            f"Generated: {len(review)} entries needing manual attention.\n\n"
        )
        for r in sorted(review, key=lambda x: x["source"] + x["title"].lower()):
            ...
            review_file.write(
                f"- **[{r['source']}]** {r['title']} — {url_str} (conf: {conf_str})\n"
            )
