...
def upload_cover(cover_url: str, title: str, slug: str) -> str | None:
    ...
    try:
        ...
        print(f"  Cover upload failed HTTP {e.code}: {body_text[:200]}", file=sys.stderr)
        return None
...
def take_backup():
    ...
    for col in collections:
        ...
        with open(backup_dir / f"{col}.json", 'w') as f:
...
def main():
    ...
    if not enriched_path.exists():
        print("ERROR: cache/psn_xbox_enriched.json not found — run enrich_psn_xbox.py first", file=sys.stderr)
        sys.exit(1)
    ...
    genre_cache = {g['name']: g['id'] for g in api_get('/items/genres?limit=-1&fields[]=id,name')['data']}
    print(f"  {len(genre_cache)} genres")

    print("Loading developer cache...")
    dev_cache = {d['name']: d['id'] for d in api_get('/items/developers?limit=-1&fields[]=id,name')['data']}
        ...
        try:
            ...
            if game.get('cover_art_url'):
                ...
                if file_uuid:
                    ...
                    print(f"  Cover upload failed — continuing without cover")
            ...
            for genre_name in game.get('genres', []):
                ...
                try:
                    api_post('/items/games_genres', {'games_id': game_id, 'genres_id': genre_id})
                except urllib.error.HTTPError as e:
                    if e.code == 400:
            ...
            for dev_name in game.get('developers', []):
                ...
                try:
                    api_post('/items/games_developers', {'games_id': game_id, 'developers_id': dev_id})
                except urllib.error.HTTPError as e:
                    if e.code == 400:
                        pass
                    else:
