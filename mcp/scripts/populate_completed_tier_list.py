...
def main():
    """Add completed games missing from the completed-games tier list."""
    ...
    completed_games = DIRECTUS.fetch_all(
    ...
    published_tl_ids = {
        tl["id"]
        for tl in DIRECTUS.fetch_all(
            "/items/tier_lists?fields=id&filter%5Bstatus%5D%5B_eq%5D=published"
        )
    } - {COMPLETED_TIER_LIST_ID}
    all_rows = DIRECTUS.fetch_all("/items/tier_rows?fields=id,label,tier_list")
    non_u_row_ids = {
        r["id"]: r["label"]
        for r in all_rows
    ...
    for game in new_games:
        ...
        if args.dry_run:
            ...
            try:
                DIRECTUS.post(
                    "/items/tier_row_games", {"tier_row_id": row_id, "game_id": gid}
                )
                print(f"  [{label}] {game['title']}", file=sys.stderr)
                added += 1
