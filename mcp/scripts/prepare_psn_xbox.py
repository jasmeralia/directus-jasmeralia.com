...
def fuzzy_match(title: str, candidates: list[str], threshold: float = 0.90) -> str | None:
    best_score = 0.0
    best = None
    for c in candidates:
...
def steam_fuzzy_match(title: str, steam_games: list[dict], threshold: float = 0.85) -> int | None:
    best_score = 0.0
    best_appid = None
...
def main():
    ...
    # --- Content filter ---
    def filter_rows(source_rows: list[dict], platform: str) -> tuple[list[dict], list[dict]]:
        kept, dropped = [], []
    ...
    with open(CACHE / 'directus_titles_current.json', 'w') as f:
    ...
    for g in all_games:
        exact = next((t for t in directus_titles if t.lower() == g['title'].lower()), None)
        if exact:
    ...
    with open(CACHE / 'steam_library.json') as f:
    ...
    for g in candidates:
        if g.get('steam_appid'):
            g['download_url'] = f"https://store.steampowered.com/app/{g['steam_appid']}/"
        elif g['platform'] == 'psn':
            g['download_url'] = f"https://store.playstation.com/en-us/search/{urllib.parse.quote(g['title'])}"
        else:
            g['download_url'] = f"https://www.xbox.com/en-US/search?q={urllib.parse.quote(g['title'])}"
    ...
    with open(CACHE / 'psn_xbox_candidates.json', 'w') as f:
        json.dump(output, f, indent=2)
    print(f"\nWrote {len(output)} candidates → cache/psn_xbox_candidates.json")

    with open(CACHE / 'psn_xbox_possible_duplicates.json', 'w') as f:
        json.dump(possible_duplicates, f, indent=2)
    print(f"Wrote {len(possible_duplicates)} possible duplicates → cache/psn_xbox_possible_duplicates.json")
