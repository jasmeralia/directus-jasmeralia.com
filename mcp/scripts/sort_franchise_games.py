...
def get_all_franchise_games():
    """Fetch every franchise-game junction with expanded game data."""
    r = requests.get(
        f"{BASE}/items/franchise_games",
        headers=HEADERS,
        params=[
            ("limit", -1),
            (
                "fields",
                "id,sort,franchise_id,game_id.id,game_id.title,game_id.release_year",
            ),
...
def get_franchise_names():
    """Fetch a mapping from franchise IDs to titles."""
    r = requests.get(
        f"{BASE}/items/franchises",
        headers=HEADERS,
...
def effective_year(game_id, db_year):
    """Return the release year after applying known corrections."""
...
def compute_new_order(rows, franchise_id):
    """Compute canonical game ordering for one franchise."""
...
def main():
    """Reorder every franchise's games into canonical order."""
    ...
    for fid in sorted(by_franchise.keys()):
        ...
        for new_sort, row in enumerate(new_order, start=1):
            if row["sort"] != new_sort:
                yr = effective_year(
                    row["game_id"]["id"], row["game_id"]["release_year"]
                )
        ...
        if franchise_updates:
            ...
            for u in franchise_updates:
                print(
                    f"  [{u['year']}] {u['title']}  ({u['old_sort']} → {u['new_sort']})"
                )
            print()
