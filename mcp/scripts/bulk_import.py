...
def upsert_developer(name: str, dev_cache: dict[str, int], dry_run: bool) -> int | None:
    ...
    try:
        ...
        if '"RECORD_NOT_UNIQUE"' in body:
            try:
                r = api("GET", f"/items/developers?filter[slug][_eq]={slug}&limit=1&fields=id,name")
                if r["data"]:
                    dev_id = r["data"][0]["id"]
                    dev_cache[name] = dev_id
                    return dev_id
...
def main():
    ...
    parser.add_argument("--delay", type=float, default=0.3, help="Seconds between games (default: 0.3)")
    ...
    devs: list[dict] = json.loads((CACHE / "backup_20260501_161932" / "developers.json").read_text())
    ...
    print(f"Total proposed: {len(proposed)} | Done: {len(done_appids)} | Pending: {len(pending)}", file=sys.stderr)
    if args.dry_run:
    ...
    for i, game in enumerate(pending):
        ...
        for genre_id in genre_ids:
            if args.dry_run:
                ...
                continue
            try:
                api("POST", "/items/games_genres", {"games_id": game_id, "genres_id": genre_id})
            except Exception as e:
        ...
        for dev_name in game.get("developers", []):
            ...
            if args.dry_run:
                ...
                continue
            try:
                api("POST", "/items/games_developers", {"games_id": game_id, "developers_id": dev_id})
            except Exception as e:
