...
**Implementation status (2026-09-04): complete.** The feature is implemented, including the previously applied schema/data flags, sitewide toggle and rendering, Recent Updates classification, split feeds, filters/chart, tests, and the maintenance script. A same-day follow-up adds color-coded review classifications, keeps tier-list titles visible while collapsing NSFW descriptions, moves the toggle below search, groups the charts with SFW/NSFW last, and flags Harem as NSFW. The optional Phase 5.2 candidate sweep remains a separately approved content follow-up; Phase 6 records the rollout procedure used to publish and validate the release.
