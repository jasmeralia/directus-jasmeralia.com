...
| `games_links` | Junction: `games_id`, `url`, `kind` (`"download" \| "walkthrough" \| "text-note" \| "other" \| "gamestorylog"`), `label` (nullable, shown for `other`-kind links without a recognized host), `sort`. Replaces the old `games.download_url`/`games.walkthrough_url` fields (see above). |
