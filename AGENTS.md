...
| Collection | Notes |
|---|---|
| `games` | Primary library. Fields: `id`, `title`, `slug`, `cover_image` (file UUID), `release_year`, `player_status`, `game_status`, `download_url`, `family_sharing` (bool), `section_style` (string, `null`/`"linear"` = ordered chapter/act/episode model, `"nonlinear"` = quest/mission pool - see the `game_sections` row below), `section_noun` (string, e.g. "Chapter"/"Act"/"Episode"/"Mission"/"Quest", null defaults to "Chapter"), `current_section` (integer, ordinal of the section currently being played; meaningful for linear games only, null = not tracked). **Always set `download_url` to `https://store.steampowered.com/app/{appid}/` when creating a Steam-sourced game.** Valid `player_status` values: `not_started`, `in_progress`, `on_hold`, `waiting_for_update`, `did_not_finish`, `completed`. Valid `game_status` values: `unreleased`, `in_development`, `abandoned`, `released`. |
